...
def sparse_convolution(x,g,n,m,M,phi,device):
    ...
    inds=((inds+n//2) % n) +n*torch.arange(0,x.shape[0],device=device,dtype=torch.long)[:,None] # % n to prevent overflows
    # next term: +n//2 for index shift from -n/2 util n/2-1 to 0 until n-1, other part for linear indices
...
def forward_nfft(x,f_hat,N,n,m,phi,phi_hat,device):
    ...
    g=torch.fft.ifftshift(torch.fft.fft(g_hat,norm="backward"),[-1]) # shift such that g lives again on [-1/2,1/2)
